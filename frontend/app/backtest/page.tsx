...
function Backtest() {
  ...
  return (
    <main className="relative min-h-screen px-4 py-5 sm:px-6 lg:px-8"><SpaceParticleField /><div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-5">
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="績效統計">
        {[["累計淨損益", money(stats.net, currency), stats.net >= 0], ["交易勝率", `${stats.winRate.toFixed(1)}%`, true], ["獲利因子", stats.profitFactor === null ? "—" : stats.profitFactor.toFixed(2), true], ["平均盈虧比", stats.winLossRatio === null ? "—" : stats.winLossRatio.toFixed(2), true], ["平均獲利 / 虧損", `${money(stats.avgWin, currency)} / ${money(stats.avgLoss, currency)}`, true], ["交易筆數", String(stats.count), true], ["交易日勝率", `${stats.dayWinRate.toFixed(1)}%`, true], ["最大回撤", money(stats.drawdown, currency), false], ["最佳交易日", money(stats.bestDay, currency), true], ["最佳日占總獲利", `${stats.bestDayShare.toFixed(1)}%`, true], ["最佳單筆交易", money(stats.best, currency), true]].map(([label, value, positive]) => <div key={String(label)} className="glass-panel rounded-xl border border-white/10 p-5"><p className="text-xs text-slate-400">{label}</p><p className={`mt-3 font-data text-xl font-bold ${positive ? "text-long" : "text-short"}`}>{value}</p></div>)}
      </section>
      <section className="glass-panel rounded-xl border border-white/10 p-5"><div className="flex items-center gap-2"><TrendingUp className="h-5 w-5 text-gold" /><h3 className="font-bold text-white">累積損益曲線</h3></div><EquityChart values={stats.curve} /></section>
