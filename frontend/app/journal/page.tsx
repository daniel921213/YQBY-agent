...
function JournalWorkspace() {
  ...
  const unpublish = async () => { if (!selected || !window.confirm("確定取消發布？讀者將無法再查看這篇日誌。")) return; try { setBusy(true); const row = await unpublishJournal(selected.id); setSelected(row); setMine((current) => current.map((item) => item.id === row.id ? row : item)); await refreshPublic(); } catch (err) { setError(err instanceof Error ? err.message : "取消發布失敗"); } finally { setBusy(false); } };
  ...
  const switchView = async (next: "mine" | "teachers") => { try { if (next === "teachers") await flush(); setView(next); setError(""); } catch (err) { setError(err instanceof Error ? err.message : "切換失敗"); } };
  ...
  return <main className="relative min-h-screen px-4 py-5 sm:px-6 lg:px-8"><SpaceParticleField /><div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-5"><PageHeader title="交易日誌" kicker="TRADING JOURNAL" />
    ...
    {view === "mine" ? <div className="grid gap-4 lg:grid-cols-[230px_minmax(0,1fr)]"><aside className="glass-panel h-fit rounded-xl border border-white/10 p-3"><button onClick={create} className="mb-3 flex w-full items-center justify-center gap-2 rounded-lg bg-gold px-3 py-2.5 text-sm font-bold text-slate-950"><Plus className="h-4 w-4" />新增日誌</button><div className="space-y-1">{mine.map((row) => <button key={row.id} onClick={async () => { try { await flush(); selectMine(row); } catch (err) { setError(err instanceof Error ? err.message : "切換失敗"); } }} className={`w-full rounded-lg px-3 py-2 text-left text-sm ${selected?.id === row.id ? "bg-gold/10 text-gold" : "text-slate-300 hover:bg-white/5"}`}><span className="block truncate">{row.title}</span><span className="text-[11px] text-slate-500">{row.is_published ? "已發布" : "私人草稿"} · {new Date(row.updated_at ?? row.created_at).toLocaleDateString("zh-TW")}</span></button>)}</div></aside>
      <section className="glass-panel min-h-[600px] rounded-xl border border-white/10 p-4 sm:p-7">{selected ? <><div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4"><span className="text-xs text-slate-400">{selected.is_published ? "已發布 · 修改後需重新發布" : "私人草稿"} · {saving ? "儲存中…" : dirty ? "等待儲存" : "已儲存"}</span><div className="flex flex-wrap gap-2">{me?.plan === "lifetime" && <button disabled={busy} onClick={publish} className="inline-flex items-center gap-2 rounded-lg bg-gold px-3 py-2 text-xs font-bold text-slate-950 disabled:opacity-50"><Send className="h-3.5 w-3.5" />{selected.is_published ? "發布更新" : "發布日誌"}</button>}{selected.is_published && <button disabled={busy} onClick={unpublish} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300">取消發布</button>}<button aria-label="刪除日誌" onClick={remove} className="rounded-lg border border-short/30 px-3 py-2 text-xs text-short"><Trash2 className="h-4 w-4" /></button></div></div>
        ...
        <RichJournal key={selected.id} journalId={selected.id} document={document} editable onChange={(next) => { setDocument(next); touch(); }} onError={setError} />
