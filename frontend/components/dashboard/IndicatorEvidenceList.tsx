        ...
        <div key={item.key} className="grid gap-3 py-4 md:grid-cols-[1fr_96px_116px] md:items-center">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm font-medium text-stone-50">{item.label}</span>
              <span className="rounded-sm border border-white/10 px-2 py-0.5 text-xs text-stone-400">
                {item.timeframe}
              </span>
            </div>
          ...
          <div className="text-sm text-stone-100">
            ...
            <span className="text-stone-500"> / {item.weight.toFixed(0)}</span>
          </div>
        </div>
      ))}
    </div>
