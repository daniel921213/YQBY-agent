...
with JWT_SECRET and carry the UID plus a login version so a password reset can
revoke every older session without changing the user's entitlement.
"""
...
def decode_token_with_version(token: str) -> tuple[str, int] | None:
    try:
...
def decode_token(token: str) -> str | None:
    """Backward-compatible UID decoder for older dependencies."""
    token_data = decode_token_with_version(token)
    return token_data[0] if token_data is not None else None
